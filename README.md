...
- data is buffered and processed by dedicated modules,
...
```mermaid
flowchart LR
    S1["sensor_node (room1,sensor15)"] --> CM["connmgr (TCP accept + validation)"]
    S2["sensor_node (room2,sensor21)"] --> CM
    S3["sensor_node (roomX,sensorY)"] --> CM

    MAP["room_sensor.map\n(room -> sensor)"] --> CM
    CM --> RAW["sensor_data_recv.txt\n(raw accepted data)"]
    CM --> SB["sbuffer (thread-safe queue)"]
    SB --> DM["datamgr\n(running average + alert state)"]
...
- `connmgr.c`
  ...
  - pushes valid measurements into `sbuffer`,
  - enforces receiver idle timeout (`N sec without data`).

- `sbuffer.c`
  - thread-safe shared queue between connmgr and datamgr,
  - current policy is blocking when full (`SBUFFER_FULL_BLOCK`) to avoid data loss.

- `datamgr.c`
  - consumes measurements from `sbuffer`,
