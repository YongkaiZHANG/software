#include <errno.h>
#include <fcntl.h>
#include <pthread.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <sys/select.h>
#include <sys/socket.h>
#include <sys/types.h>
#include <time.h>
#include <unistd.h>
#include <sys/mman.h>
#include <sys/wait.h>
#include <signal.h>
#include "config.h"
#include "connmgr.h"
#include "lib/tcpsock.h"
#include "sbuffer.h"

#ifndef TIMEOUT
#error TIMEOUT not defined
#endif

typedef struct {
    uint16_t room_id;
    sensor_id_t sensor_id;
} sensor_map_entry_t;

typedef struct {
    unsigned long long total_received;
    unsigned long long total_dropped;
    unsigned long long total_rejected;
    time_t last_data_timestamp;
    int active_clients;
    pthread_mutex_t state_mutex;
} connmgr_stats_t;

typedef struct client_node {
    pid_t pid;
    struct client_node *next;
} client_node_t;

static sbuffer_t **shared_buffer = NULL;
static connmgr_stats_t *stats = NULL;
static client_node_t *client_list_head = NULL;
static FILE *receiver_data_file = NULL;
static sensor_map_entry_t *sensor_map_entries = NULL;
static size_t sensor_map_count = 0;

static void free_sensor_map(void)
{
    free(sensor_map_entries);
    sensor_map_entries = NULL;
    sensor_map_count = 0;
}

static int load_sensor_map(void)
{
    FILE *map_file;
    sensor_map_entry_t *entries;
    size_t capacity = 16;
    size_t count = 0;
    uint16_t room_id;
    uint16_t sensor_id;

    map_file = fopen("room_sensor.map", "r");
    if (map_file == NULL) {
        return -1;
    }

    entries = malloc(capacity * sizeof(*entries));
    if (entries == NULL) {
        fclose(map_file);
        return -1;
    }

    while (fscanf(map_file, "%hu %hu", &room_id, &sensor_id) == 2) {
        if (count == capacity) {
            sensor_map_entry_t *resized;

            capacity *= 2;
            resized = realloc(entries, capacity * sizeof(*entries));
            if (resized == NULL) {
                free(entries);
                fclose(map_file);
                return -1;
            }
            entries = resized;
        }

        entries[count].room_id = room_id;
        entries[count].sensor_id = (sensor_id_t)sensor_id;
        count++;
    }

    fclose(map_file);
    if (count == 0) {
        free(entries);
        return -1;
    }

    free_sensor_map();
    sensor_map_entries = entries;
    sensor_map_count = count;
    return 0;
}

static bool is_valid_sensor_pair(uint16_t room_id, sensor_id_t sensor_id)
{
    for (size_t i = 0; i < sensor_map_count; i++) {
        if (sensor_map_entries[i].room_id == room_id &&
            sensor_map_entries[i].sensor_id == sensor_id) {
            return true;
        }
    }
    return false;
}

static void log_rejected_pair(uint16_t room_id, sensor_id_t sensor_id)
{
    int fd = open(FIFO_LOG, O_WRONLY);
    if (fd < 0) return;

    FILE *log_file = fdopen(fd, "w");
    if (log_file == NULL) {
        close(fd);
        return;
    }

    fprintf(
        log_file,
        "REJECT_INVALID_PAIR room=%hu sensor=%hu reason=not_in_map\n",
        room_id,
        sensor_id
    );
    fclose(log_file);
}

static void init_runtime_stats(void)
{
    stats = mmap(NULL, sizeof(*stats), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if (stats != MAP_FAILED) {
        pthread_mutexattr_t mattr;
        pthread_mutexattr_init(&mattr);
        pthread_mutexattr_setpshared(&mattr, PTHREAD_PROCESS_SHARED);
        pthread_mutex_init(&stats->state_mutex, &mattr);
        pthread_mutexattr_destroy(&mattr);

        stats->active_clients = 0;
        stats->total_received = 0;
        stats->total_dropped = 0;
        stats->total_rejected = 0;
        stats->last_data_timestamp = time(NULL);
    }
}

static void cleanup_runtime_stats(void)
{
    if (stats && stats != MAP_FAILED) {
        pthread_mutex_destroy(&stats->state_mutex);
        munmap(stats, sizeof(*stats));
        stats = NULL;
    }
}

static int get_active_clients(void)
{
    int count = 0;
    if (stats) {
        pthread_mutex_lock(&stats->state_mutex);
        count = stats->active_clients;
        pthread_mutex_unlock(&stats->state_mutex);
    }
    return count;
}

static time_t get_last_data_timestamp(void)
{
    time_t timestamp = 0;
    if (stats) {
        pthread_mutex_lock(&stats->state_mutex);
        timestamp = stats->last_data_timestamp;
        pthread_mutex_unlock(&stats->state_mutex);
    }
    return timestamp;
}

static void update_receive_stats(bool dropped)
{
    if (stats) {
        pthread_mutex_lock(&stats->state_mutex);
        stats->total_received++;
        if (dropped) stats->total_dropped++;
        stats->last_data_timestamp = time(NULL);
        pthread_mutex_unlock(&stats->state_mutex);
    }
}

static void update_rejected_stats(void)
{
    if (stats) {
        pthread_mutex_lock(&stats->state_mutex);
        stats->total_rejected++;
        pthread_mutex_unlock(&stats->state_mutex);
    }
}

static void append_receiver_measurement(const sensor_data_t *data)
{
    if (data == NULL) return;

    if (stats) pthread_mutex_lock(&stats->state_mutex);
    if (receiver_data_file != NULL) {
        fprintf(
            receiver_data_file,
            "%hu %hu %.2f %ld\n",
            data->room_id,
            data->sensor_id,
            data->value,
            (long)data->timestamp
        );
        fflush(receiver_data_file);
    }
    if (stats) pthread_mutex_unlock(&stats->state_mutex);
}

static void shutdown_all_clients(void)
{
    client_node_t *node = client_list_head;
    while (node != NULL) {
        kill(node->pid, SIGTERM);
        node = node->next;
    }
}

static int receive_measurement(tcpsock_t *client, sensor_data_t *data)
{
    int bytes;
    int rc;

    bytes = sizeof(data->sensor_id);
    rc = tcp_receive(client, &data->sensor_id, &bytes);
    if (rc != TCP_NO_ERROR) return rc;

    bytes = sizeof(data->room_id);
    rc = tcp_receive(client, &data->room_id, &bytes);
    if (rc != TCP_NO_ERROR) return rc;

    bytes = sizeof(data->value);
    rc = tcp_receive(client, &data->value, &bytes);
    if (rc != TCP_NO_ERROR) return rc;

    bytes = sizeof(data->timestamp);
    rc = tcp_receive(client, &data->timestamp, &bytes);
    if (rc != TCP_NO_ERROR) return rc;

    data->RUN_AVG = 0;
    data->sample_count = 0;
    data->alert_state = 0;
    for (size_t i = 0; i < RUN_AVG_LENGTH; i++) {
        data->temperatures[i] = 0;
    }
    return TCP_NO_ERROR;
}

static void client_process(tcpsock_t *client)
{
    sensor_data_t data;

    while (receive_measurement(client, &data) == TCP_NO_ERROR) {
        int insert_rc;

        if (!is_valid_sensor_pair(data.room_id, data.sensor_id)) {
            log_rejected_pair(data.room_id, data.sensor_id);
            update_rejected_stats();
            tcp_close(&client);
            exit(EXIT_FAILURE);
        }

        append_receiver_measurement(&data);
        insert_rc = sbuffer_insert(*shared_buffer, &data);
        if (insert_rc == SBUFFER_FAILURE) break;
        update_receive_stats(insert_rc == SBUFFER_DROPPED);
    }

    tcp_close(&client);
    exit(EXIT_SUCCESS);
}

static void sigchld_handler(int sig)
{
    int saved_errno = errno;
    pid_t pid;
    while ((pid = waitpid(-1, NULL, WNOHANG)) > 0) {
        if (stats) {
            pthread_mutex_lock(&stats->state_mutex);
            if (stats->active_clients > 0) {
                stats->active_clients--;
            }
            pthread_mutex_unlock(&stats->state_mutex);
        }
    }
    errno = saved_errno;
}

void connmgr_listen(sbuffer_t *sbuffer, int port, int timeout_seconds)
{
    tcpsock_t *server = NULL;
    int server_sd;

    if (sbuffer == NULL) return;
    if (port <= 0) return;
    if (timeout_seconds < 0) timeout_seconds = 30;

    shared_buffer = &sbuffer;
    init_runtime_stats();

    struct sigaction sa;
    sa.sa_handler = sigchld_handler;
    sigemptyset(&sa.sa_mask);
    sa.sa_flags = SA_RESTART | SA_NOCLDSTOP;
    sigaction(SIGCHLD, &sa, NULL);

    if (load_sensor_map() != 0) {
        fprintf(stderr, "Unable to load room_sensor.map for validation\n");
        sbuffer_close(sbuffer);
        return;
    }

    receiver_data_file = fopen(RECEIVER_DATA_LOG, "w");
    if (receiver_data_file != NULL) {
        setvbuf(receiver_data_file, NULL, _IOLBF, 0);
    } else {
        perror("fopen sensor_data_recv.txt");
    }

    if (tcp_passive_open(&server, port) != TCP_NO_ERROR) {
        fprintf(stderr, "Unable to start TCP server on port %d\n", port);
        if (receiver_data_file != NULL) {
            fclose(receiver_data_file);
            receiver_data_file = NULL;
        }
        free_sensor_map();
        sbuffer_close(sbuffer);
        return;
    }

    if (tcp_get_sd(server, &server_sd) != TCP_NO_ERROR) {
        tcp_close(&server);
        if (receiver_data_file != NULL) {
            fclose(receiver_data_file);
            receiver_data_file = NULL;
        }
        free_sensor_map();
        sbuffer_close(sbuffer);
        return;
    }

    if (timeout_seconds == 0) {
        printf("Connection manager listening on port %d (idle timeout: disabled)\n", port);
    } else {
        printf(
            "Connection manager listening on port %d (idle timeout after %d sec without data)\n",
            port,
            timeout_seconds
        );
    }

    while (1) {
        fd_set readfds;
        struct timeval poll_timeout;
        int ready;

        FD_ZERO(&readfds);
        FD_SET(server_sd, &readfds);
        poll_timeout.tv_sec = 1;
        poll_timeout.tv_usec = 0;

        ready = select(server_sd + 1, &readfds, NULL, NULL, &poll_timeout);
        if (ready < 0) {
            if (errno == EINTR) continue;
            perror("select");
            break;
        }

        if (ready > 0 && FD_ISSET(server_sd, &readfds)) {
            tcpsock_t *client = NULL;
            pid_t pid;

            if (tcp_wait_for_connection(server, &client) != TCP_NO_ERROR) {
                fprintf(stderr, "Failed to accept an incoming connection\n");
                continue;
            }

            if (stats) {
                pthread_mutex_lock(&stats->state_mutex);
                stats->active_clients++;
                pthread_mutex_unlock(&stats->state_mutex);
            }

            pid = fork();
            if (pid < 0) {
                perror("fork");
                tcp_close(&client);
                if (stats) {
                    pthread_mutex_lock(&stats->state_mutex);
                    stats->active_clients--;
                    pthread_mutex_unlock(&stats->state_mutex);
                }
            } else if (pid == 0) {
                // Child process
                tcp_close(&server); // Close listening socket
                client_process(client);
                // Exit is called in client_process
            } else {
                // Parent process
                tcp_close(&client); // Close client socket
                client_node_t *node = malloc(sizeof(client_node_t));
                if (node) {
                    node->pid = pid;
                    node->next = client_list_head;
                    client_list_head = node;
                }
            }
        }

        if (timeout_seconds > 0) {
            time_t now = time(NULL);
            time_t last_data = get_last_data_timestamp();

            if (now - last_data >= timeout_seconds) {
                printf(
                    "Connection manager idle timeout reached: %d seconds without incoming data\n",
                    timeout_seconds
                );
                break;
            }
        }
    }

    shutdown_all_clients();
    sbuffer_close(sbuffer);

    while (get_active_clients() > 0) {
        usleep(100000);
    }

    if (server != NULL) {
        tcp_close(&server);
    }

    if (stats) {
        pthread_mutex_lock(&stats->state_mutex);
        if (receiver_data_file != NULL) {
            fclose(receiver_data_file);
            receiver_data_file = NULL;
        }
        printf(
            "Connection manager stopped. total received=%llu, dropped=%llu, rejected=%llu (queue dropped=%llu)\n",
            stats->total_received,
            stats->total_dropped,
            stats->total_rejected,
            sbuffer_get_drop_count(sbuffer)
        );
        pthread_mutex_unlock(&stats->state_mutex);
    }
    
    // Free client list
    client_node_t *curr = client_list_head;
    while (curr != NULL) {
        client_node_t *next = curr->next;
        free(curr);
        curr = next;
    }
    client_list_head = NULL;

    free_sensor_map();
    cleanup_runtime_stats();
    return;
}

void connmgr_free(void)
{
    // Do nothing as resources are cleaned up before connmgr_listen returns
}